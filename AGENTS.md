...
- 已交付主线统一登记在 `scripts/course_catalog.py`；后续路线在 PLAN.md 写清问题和验收再实现。新单元通过验收后登记，使生成、执行与结构检查共用同一清单。
...
- 运行结构与链接检查、受影响测试、受影响已交付单元的 notebook 和 documented CLI；共享闭环改动需回归第一单元。
...
- 每阶段通过本机验证与两类独立审查后，单独提交并推送GitHub；核对远端SHA与该提交的CI，失败要修复。提交只包含该阶段已经验收的文件，不混入并行中的后续单元。
